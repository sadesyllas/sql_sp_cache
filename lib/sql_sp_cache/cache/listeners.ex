defmodule SqlSpCache.Cache.Listeners do
  ...
  def get_all_clients(cache_key)
  do
    GenServer.call(@mod, {:get_all_clients, cache_key})
  end
  ...
  def handle_call({:get_all_clients, cache_key}, _from, state)
  do
    clients = MapSet.to_list(state.cache_key_client[cache_key] || MapSet.new())
    clients = clients ++ MapSet.to_list(state.cache_key_client_once[cache_key] || MapSet.new())
    {:reply, clients, state}
  end
  ...
  def handle_call({:add_once, {client, cache_key}}, _from,
  ...
  do
    ...
    cache_key_client_once_new = Enum.reduce(cache_key_client_once, %{}, fn {cache_key, client_set}, acc ->
      new_client_set = MapSet.delete(client_set, client)
      case MapSet.size(new_client_set) do
        0 -> Map.delete(acc, cache_key)
        _ -> Map.put(acc, cache_key, new_client_set)
    ...
    new_state =
      ...
      |> Map.put(:client_cache_key, client_cache_key_new)
      |> Map.put(:cache_key_client, cache_key_client_new)
      |> Map.put(:cache_key_client_once, cache_key_client_once_new)
    {:reply, new_state, new_state}
  end
  ...
  do
    ...
    cache_key_client_once_new = Map.delete(cache_key_client_once, cache_key)
    new_state =
      state
      |> Map.put(:client_cache_key, client_cache_key_new)
      |> Map.put(:cache_key_client, cache_key_client_new)
